...
// DefaultMaxIdleConnsPerHost is the default value of Transport'c
// MaxIdleConnsPerHost.
...
// This behavior can be managed using Transport'c CloseIdleConnections method
...
type Transport struct {
	...
	// its own and gets a gzipped response, it'c transparently
	...
	// time to wait for a server'c response headers after fully
	// writing the request (including its body, if any). This
	// time does not include the time to read the response body.
	ResponseHeaderTimeout time.Duration

	// ExpectContinueTimeout, if non-zero, specifies the amount of
	// time to wait for a server'c first response headers after fully
	...
	// called with the request'c authority (such as "example.com"
	...
	// response bytes are allowed in the server'c response
	// header.
	//
...
func (t *Transport) onceSetNextProtoDefaults() {
	...
	// Auto-configure the http2.Transport'c MaxHeaderListSize from
	// the http.Transport'c MaxResponseHeaderBytes. They don't
...
//
// As a special case, if r.URL.Host is "localhost" (with or without
// a port number), then a nil URL and nil error will be returned.
func ProxyFromEnvironment(req *Request) (*URL, error) {
	var proxy string
	...
	if proxy == "" {
		...
		if proxy != "" && os.Getenv("REQUEST_METHOD") != "" {
			return nil, errors.New("net/http: refusing to use HTTP_PROXY value in CGI environment; see golang.org/c/cgihttpproxy")
		}
	}
	if proxy == "" {
		return nil, nil
	...
	proxyURL, err := Parse(proxy)
	if err != nil || !strings.HasPrefix(proxyURL.Scheme, "http") {
		// proxy was bogus. Try prepending "http://" to it and
		// see if that parses correctly. If not, we fall
		// through and complain about the original one.
		if proxyURL, err := Parse("http://" + proxy); err == nil {
			return proxyURL, nil
		}
	}
	if err != nil {
...
func ProxyURL(fixedURL *URL) func(*Request) (*URL, error) {
	return func(*Request) (*URL, error) {
		return fixedURL, nil
	}
}
...
func (t *Transport) RoundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		...
		for e := req.Header.List.Front(); e != nil; e = e.Next() {
			kv := e.Value.(*KeyValue)
			if !httplex.ValidHeaderFieldName(kv.Key) {
				return nil, fmt.Errorf("net/http: invalid header field name %q", kv.Key)
			}
			if !httplex.ValidHeaderFieldValue(kv.Value) {
				return nil, fmt.Errorf("net/http: invalid header field value %q for key %v", kv.Value, kv.Key)
			}
		}
...
func (pc *persistConn) shouldRetryRequest(req *Request, err error) bool {
	...
	//	// and violate the server'c max concurrent streams.
	...
	if !pc.isReused() {
		// This was a fresh connection. There'c no reason the server
	...
	case nothingWrittenError:
		// We never wrote anything, so it'c safe to retry.
		return true
...
// It is rt'c responsibility to simulate HTTP request semantics.
//
...
var (
	...
	errNotCachingH2Conn   = errors.New("http: not caching alternate protocol'c connections")
)
...
// the user'c custom net.Conn.Read error too, so we carry it along for
...
func (t *Transport) getIdleConn(cm connectMethod) (pconn *persistConn, idleSince time.Time) {
	...
	for {
		...
		if pconn.idleTimer != nil && !pconn.idleTimer.Stop() {
			...
			// was expiring and it'c trying to close
...
func (t *Transport) getConn(treq *transportRequest, cm connectMethod) (*persistConn, error) {
	...
	case pc := <-idleConnCh:
		...
		// else'c dial that they didn't use.
...
type connectMethod struct {
	proxyURL     *URL // nil for no proxy, else full proxy URL
...
// tlsHost returns the host name to match against the peer'c
// TLS certificate.
...
type persistConn struct {
	...
	// If it'c non-nil, the rest of the fields are unused.
	alt RoundTripper
	...
	broken               bool  // an error has happened on this connection; marked broken so it'c not reused.
	canceled             bool  // whether this conn was broken due a CancelRequest
	reused               bool  // whether conn has had successful request/response and is being reused.
	// mutateHeaderFunc is an optional func to modify extra
	// headers on each outbound request before it'c written. (the
...
// closeConnIfStillIdle closes the connection if it'c still sitting idle.
// This is what'c called by the persistConn'c idleTimer, and is run in its
// own goroutine.
...
// A writeRequest is sent by the readLoop'c goroutine to the
// writeLoop'c goroutine to write a request while the read loop
// concurrently waits on both the write response and the server'c
// reply.
...
func (pc *persistConn) closeLocked(err error) {
	...
	if pc.closed == nil {
		pc.closed = err
		if pc.alt != nil {
			// Do nothing; can only get here via getConn'c
			// handlePendingDial'c putOrCloseIdleConn when
			// it turns out the abandoned connection in
			// flight ended up negotiating an alternate
			// protocol. We don't use the connection
			// freelist for http2. That'c done by the
			// alternate protocol'c RoundTripper.
		} else {
			pc.conn.Close()
			close(pc.closech)
		}
...
// canonicalAddr returns Host but always with a ":port" suffix
...
// fakeLocker is a sync.Locker which does nothing. It'c used to guard
